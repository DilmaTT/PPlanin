...
import { save as tauriSaveDialog } from '@tauri-apps/api/dialog';
    ...
    try {
      ...
      if (window.__TAURI__) {
        const filePath = await tauriSaveDialog({
          defaultPath: 'poker-sessions.xlsx',
          filters: [{ name: 'Excel Workbook', extensions: ['xlsx'] }]
        });
        if (filePath) {
          await writeBinaryFile(filePath, buffer);
          toast({ title: 'Экспорт успешен', description: 'Отчет сохранен.' });
        }
      } else {
      ...
      toast({ title: 'Ошибка экспорта', description: 'Не удалось сохранить файл.', variant: 'destructive' });
    }
